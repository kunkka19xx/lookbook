...
```bash
cp sources/niri/*.toml ~/.look/sources/
mkdir -p ~/.look/bin && cp sources/niri/bin/* ~/.look/bin/ && chmod +x ~/.look/bin/niri-*
```
...
The daemon has to be running before any row here can do anything: `swww img` talks to it rather than drawing by itself. If the wallpaper does not come back on its own, add `swww restore` after it.
...
- **A different viewer.** `imv {path}`, `swayimg {path}` or `gwenview {path}`. Keep the `niri msg action spawn --` in front of any of them.

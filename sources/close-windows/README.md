...
Two files, one per platform. **`close-windows.toml`** is Linux, and closes every window except Look's. **`close-windows-macos.toml`** is macOS, and quits every app except Finder, because apps are the unit there: one with no windows is still running and still in the Dock.
...
```bash
cp sources/close-windows/*.toml ~/.look/sources/
```
